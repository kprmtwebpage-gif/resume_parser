...
# ── Job Applications API (alternative endpoint) ────────────────
...
async def upload_editor_image(image: UploadFile = File(...)):
    ...
    editor_dir = Path("uploads/editor")
...
async def get_candidates(
    ...
    experienceFrom: Optional[int] = Query(None, ge=0, le=50, description="Minimum years of experience"),
    experienceTo: Optional[int] = Query(None, ge=0, le=50, description="Maximum years of experience"),
    experienceStatus: Optional[str] = Query(None, description="Experience status: current, past, current_and_past"),
    ...
    - **keywords**: Search by skills (comma-separated, searches in tech_skills)
    - **experienceFrom**: Minimum years of experience
    - **experienceTo**: Maximum years of experience
    - **experienceStatus**: Experience status filter (current, past, current_and_past)
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            # Keywords/Skills search - search in tech_skills field
            if keywords:
            ...
            if experienceFrom is not None:
                where_conditions.append("COALESCE(s.years_of_experience, 0) >= %s")
                search_params.append(experienceFrom)
            
            if experienceTo is not None:
                where_conditions.append("COALESCE(s.years_of_experience, 0) <= %s")
                search_params.append(experienceTo)
...
async def get_all_job_titles(
    ...
    """
    Get all unique job titles from the database for dropdown population.
    
    - **limit**: Maximum number of results to return (default 500)
    """
    with get_db() as conn:
        with conn.cursor() as cursor:
            # Get unique job titles from the skills profile table
            cursor.execute(f"""
                SELECT DISTINCT job_title 
                FROM {SKILLS_TABLE}
                WHERE job_title IS NOT NULL AND job_title != ''
...
async def export_resumes(ids: str = Query(..., description="Comma-separated candidate IDs to export"), background_tasks: BackgroundTasks = None):
    """
    Export resumes for the given comma-separated candidate IDs as a ZIP archive.

    Example: /candidates/export?ids=1,2,3
    """
    # Parse IDs
    try:
        id_list = [int(x) for x in ids.split(",") if x.strip()]
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid ids parameter. Provide comma-separated integers.")

    if not id_list:
        raise HTTPException(status_code=400, detail="No candidate IDs provided")

    # Fetch resume filenames for these candidates
    with get_db() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                f"SELECT id, resume_filename FROM {CANDIDATES_TABLE} WHERE id = ANY(%s)",
                (id_list,)
            )
            rows = cursor.fetchall()
    ...
    with zipfile.ZipFile(tmp_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for row in rows:
            ...
            if not fname:
                continue
            resume_path = os.path.join(os.path.dirname(__file__), fname)
            if os.path.exists(resume_path):
                # Use candidate id + original filename to avoid collisions
                arcname = f"{row.get('id')}_{os.path.basename(resume_path)}"
                try:
                    zf.write(resume_path, arcname=arcname)
                    added += 1
                except Exception:
    ...
    if added == 0:
        ...
        try:
            ...
            pass
        raise HTTPException(status_code=404, detail="No resume files available to export for the provided IDs")
    ...
    return FileResponse(path=tmp_path, media_type='application/zip', filename='resumes_export.zip')
