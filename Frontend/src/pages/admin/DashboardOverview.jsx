...
function generateSampleData() {
  return {
    ...
    login_activity: [], login_weekly: [], login_monthly: [],
    upload_trend: [], upload_weekly: [], upload_monthly: [],
    user_performance: [], recent_activity: [],
  }
}
...
export default function DashboardOverview() {
  ...
  const [stats, setStats] = useState(() => generateSampleData())
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(apiUrl('/api/auth/admin/dashboard-stats'), { headers: getAuthHeaders() })
      if (res.ok) {
