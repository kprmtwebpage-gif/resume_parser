...
export default function SendEmailPanel({ isOpen, onClose, candidateId, candidateName, provider }) {
  ...
  useEffect(() => {
    ...
    setContacts([])
    // Only fetch companies for client/vendor/own_company modes
  ...
  return (
    <>
      ...
      >
        ...
        {recipientType !== 'candidate' && (
          <>
            ...
            <div style={{ marginBottom: '18px' }}>
              ...
              >
                <option value="">
                  {loadingCompanies ? 'Loading...' : `— Select a ${recipientType === 'vendor' ? 'vendor' : 'company'} —`}
                </option>
                {companies.map(c => (
              ...
              {!loadingCompanies && companies.length === 0 && (
                <div style={{ fontSize: '12px', color: '#f59e0b', marginTop: '6px' }}>
                  No {recipientType === 'vendor' ? 'vendor' : recipientType === 'own_company' ? 'own company' : 'client'} companies found.
                  {' '}Create one in Customer → set Category to {recipientType === 'vendor' ? 'Vendor' : recipientType === 'own_company' ? 'Own Company' : 'Client'}.
                </div>
              )}
            </div>
        ...
        {/* ── CANDIDATE EMAIL — Templates only (centered) ──── */}
        {recipientType === 'candidate' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {/* Candidate info box */}
            <div style={{
              display: 'flex', alignItems: 'center', gap: '12px',
              padding: '12px 14px', marginBottom: '20px', width: '100%',
              background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px',
            }}>
              <div style={{
                width: '36px', height: '36px', borderRadius: '50%',
                background: '#dcfce7', display: 'flex', alignItems: 'center',
                justifyContent: 'center', fontSize: '15px', color: '#16a34a',
