/**
 * AdminLayout — Sidebar + Top Header shell for the admin dashboard.
 * Uses DashboardContext for user info, logout, logo, and homeHref.
 * All routing is handled by the parent <AdminDashboard> / <Routes>.
 */
...
import {
  Users, Clock, LogOut,
  Menu, ChevronLeft, ChevronDown, ArrowLeft, RefreshCw, Bell,
  LayoutDashboard, Upload, Home,
} from 'lucide-react'

export default function AdminLayout() {
  const { user, onLogout, logo, homeHref } = useDashboard()
  ...
  /* ── isActive: path-agnostic, works at any mount point ─── */
  const base = resolvedRoot.pathname.replace(/\/$/, '') // e.g. '/admin' or '/dashboard'
  ...
  const dashboardSubmenus = [
    ...
    { path: 'upload-metrics',  label: 'Upload Metrics', icon: Upload },
  ]
  const menuItems = [
  ...
  const SidebarContent = () => (
    <>
          ...
          {logo ? (
            <img
              ...
              className={`object-contain transition-all duration-300 ${
                sidebarOpen ? 'h-14 w-auto max-w-[190px]' : 'h-10 w-10 rounded-lg'
              }`}
            />
          ) : (
            <div className={`flex items-center justify-center rounded-xl bg-blue-600 text-white font-extrabold transition-all ${
              sidebarOpen ? 'h-10 w-10 text-lg' : 'h-10 w-10 text-lg'
            }`}>A</div>
          )}
          {sidebarOpen && (
            <span className="text-[10px] font-medium tracking-[0.18em] uppercase text-gray-400 pl-0.5">
              Admin Panel
            </span>
          )}
        </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {sidebarOpen && active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-blue-400" />}
            </button>
          )
        })}
      </nav>
      ...
      <div className="px-3 mt-1">
        <button
          onClick={() => { navigate(homeHref || '/'); setMobileSidebarOpen(false) }}
          className={`group w-full flex items-center gap-3 rounded-lg transition-all duration-200 ${
            sidebarOpen ? 'px-3 py-2.5' : 'px-2 py-2.5 justify-center'
        ...
        {sidebarOpen && (
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[13px] font-extrabold text-gray-100 truncate tracking-tight" style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
                {user?.username || 'Admin'}
              </p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="bg-white border-b px-6 h-[60px] flex items-center justify-between gap-4 shrink-0" style={{ borderColor: '#E5E7EB' }}>
          <div className="flex items-center gap-4">
            <button onClick={() => setMobileSidebarOpen(true)} className="p-2 -ml-2 hover:bg-gray-100 rounded-lg transition-colors lg:hidden">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => window.location.reload()} className="p-2 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors" title="Refresh">
              <RefreshCw className="h-[18px] w-[18px]" />
            </button>
            <button className="relative p-2 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors" title="Notifications">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-[13px] font-extrabold tracking-tight leading-none mt-0.5" style={{ color: '#111827', fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
                  {user?.username || 'Admin'}
                </p>
              </div>
            </div>
