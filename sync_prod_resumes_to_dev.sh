#!/bin/bash
#############################################################################
# Full Mirror: Production Resumes → DEV
#
# - Copies ALL files from production to dev
# - Removes files in dev that don't exist in production
# - Production volume is NEVER written to (read-only mount)
# - Running production containers are completely unaffected
#
# Usage:
#   bash sync_prod_resumes_to_dev.sh
#############################################################################
...
TARGET_VOLUME="resume-dev_resume_cache"
LOG_FILE="/tmp/sync_dev_$(date +%Y%m%d_%H%M%S).log"
...
echo -e "${CYAN}==========================================${NC}"
echo -e "${CYAN}Full Mirror: Production → DEV${NC}"
echo -e "${CYAN}==========================================${NC}"
echo ""
echo "  Log: $LOG_FILE"
echo ""
...
if [ "$AFTER" -eq "$PROD_COUNT" ]; then
    ...
    echo -e "${RED}  ⚠ Mismatch: prod=$PROD_COUNT dev=$AFTER — re-run if needed${NC}"
fi
